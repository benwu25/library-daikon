...
fn baz(c_arr: &[&C]) {
    ...
    dtrace_print_pointer(
        c_arr as *const _ as *const () as usize,
        String::from("c_arr"),
    );
...
fn foo_bar(b_arr: &[&B]) {
    ...
    dtrace_print_pointer(
        b_arr as *const _ as *const () as usize,
        String::from("b_arr"),
    );
...
fn bar_foo(d_arr: &[&D]) {
    ...
    dtrace_print_pointer(
        d_arr as *const _ as *const () as usize,
        String::from("d_arr"),
    );
...
fn bar_bar(cont_arr: &Vec<&Container>) {
    ...
    dtrace_print_pointer_vec::<Container>(
        cont_arr,
        format!("{}{}", String::from("cont_arr"), "[..]"),
    );
    Container::dtrace_print_fields_vec(
        cont_arr,
        3,
        format!("{}{}", String::from("cont_arr"), "[..]"),
    );
...
fn a_vec_depth3(a_vec: &Vec<&A>) {
    dtrace_entry(
        "a_vec_depth3:::ENTER",
        *A_VEC_DEPTH3_COUNTER.lock().unwrap(),
    );
...
fn a_vec_depth2(a_vec: &Vec<&A>) {
    dtrace_entry(
        "a_vec_depth2:::ENTER",
        *A_VEC_DEPTH2_COUNTER.lock().unwrap(),
    );
    dtrace_print_pointer(a_vec.as_ptr() as usize, String::from("a_vec"));
    dtrace_print_pointer_vec::<A>(a_vec, format!("{}{}", String::from("a_vec"), "[..]"));
...
fn a_vec_depth1(a_vec: &Vec<&A>) {
    dtrace_entry(
        "a_vec_depth1:::ENTER",
        *A_VEC_DEPTH1_COUNTER.lock().unwrap(),
    );
    dtrace_print_pointer(a_vec.as_ptr() as usize, String::from("a_vec"));
    dtrace_print_pointer_vec::<A>(a_vec, format!("{}{}", String::from("a_vec"), "[..]"));
...
// dtrace File
static tr: LazyLock<Mutex<Option<File>>> = LazyLock::new(|| Mutex::new(dtrace_open()));
